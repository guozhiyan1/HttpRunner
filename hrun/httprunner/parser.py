...
def get_mapping_variable(variable_name, variables_mapping):
    ...
    try:
        ...
        print(f"{variable_name} is not found.")
...
def __prepare_config(config, project_mapping, session_variables_set=None):
    ...
    override_variables = utils.deepcopy_dict(project_mapping.get("variables", {}))  # < - - - deepcopy_dict确保deep_copy可以成功
    functions = project_mapping.get("functions", {})

    if isinstance(raw_config_variables, str) and function_regex_compile.match(  # < - - - 正则匹配变量中的方法${func1($var_1, $var_3)}
    ...
    raw_config_variables_mapping.update(override_variables)  # < - - - 确认变量格式准确

    if raw_config_variables_mapping:
        config["variables"] = raw_config_variables_mapping  # < - - - 如果有变量则更新config的变量值。因此这里说明了variables非必输项

    check_variables_set = set(raw_config_variables_mapping.keys())
    check_variables_set |= (session_variables_set or set())  # < - - - 没有则为空
    #prepare_lazy_data这一步很重要，把变量中需要调用方法的值标注成了hrun中的格式。${generate_random_str()} ---> LazyString(${generate_random_str()})
...
def __prepare_testcase_tests(tests, config, project_mapping, session_variables_set=None):
    ...
    for test_dict in tests:
        ...
        # 用testcase的config中的配置 覆盖testcase中的字段
        ...
        # 用testcase中的config覆盖 基础的testcase中的config
...
def _parse_testcase(testcase, project_mapping, session_variables_set=None):
    ...
    try:
        prepared_config = __prepare_config(  # < - - - 解析配置
            testcase["config"],
            project_mapping,
            session_variables_set
        )
        prepared_testcase_tests = __prepare_testcase_tests(  # < - - - 解析步骤
            testcase["teststeps"],
            prepared_config,
            project_mapping,
            session_variables_set
    ...
    for test_type in tests_mapping:
        ...
        if test_type == "testsuites":
            ...
            for testsuite in testsuites:
                parsed_testcases = _parse_testsuite(testsuite, project_mapping)  # < - - - 解析testsuite
            ...
            for testcase in tests_mapping["testcases"]:
                ...
                parsed_testcase = _parse_testcase(testcase, project_mapping)  # < - - - 解析testcase
                if not parsed_testcase:
                    continue
                testcases.append(parsed_testcase)
            ...
            for api_content in tests_mapping["apis"]:
                ...
                parsed_testcase = _parse_testcase(testcase, project_mapping)  # < - - - 解析api
                if not parsed_testcase:
                    continue
                testcases.append(parsed_testcase)
